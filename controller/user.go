...
func Login(c *fiber.Ctx) error {
	...
	if result := database.DB.Where("email=?",input.Email).First(&user); 
	...
	//Compare password 
	if err := bcrypt.CompareHashAndPassword([]byte(user.Password),[]byte(input.Password)); 
...
func LoginAdmin(c *fiber.Ctx) error {
	...
	if result := database.DB.Where("email=?",input.Email).First(&admin); 
	...
	//Compare password 
	if err := bcrypt.CompareHashAndPassword([]byte(admin.Password),[]byte(input.Password)); 
